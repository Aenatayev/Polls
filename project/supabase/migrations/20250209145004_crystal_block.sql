/*
  # Form Builder Schema Update
  
  1. New Tables
    - `form_templates` - Stores reusable form templates
    - `form_sections` - Allows grouping of questions
    - `form_validations` - Custom validation rules
    - `form_themes` - Custom styling/themes
  
  2. Changes
    - Add new columns to polls and surveys tables
    - Add support for conditional logic
    - Add support for scoring/points
...
-- Add foreign key constraints for form sections
ALTER TABLE form_sections
  ADD CONSTRAINT form_sections_poll_fk 
    FOREIGN KEY (form_id) 
    REFERENCES polls(id) 
    ON DELETE CASCADE,
  ADD CONSTRAINT form_sections_survey_fk 
    FOREIGN KEY (form_id) 
...
-- Add foreign key constraints for form validations
ALTER TABLE form_validations
  ADD CONSTRAINT form_validations_poll_fk 
    FOREIGN KEY (question_id) 
    REFERENCES questions(id) 
    ON DELETE CASCADE,
  ADD CONSTRAINT form_validations_survey_fk 
    FOREIGN KEY (question_id) 
